...
require_once plugin_dir_path(__FILE__) . 'GetWinddataOld.php';
require_once plugin_dir_path(__FILE__) . 'GetWindoptionsOld.php';
$winddata = new GetWinddataOld();
$windoptions = new GetWindoptionsOld();
...
<section class="single-text">
  <div class="container">
        ...
        <?php 
          ...
          if (get_locale() == 'ru_RU') { 
            $link = get_permalink( get_page_by_path( 'winddata' ) );
          } else { 
            $link = get_permalink( get_page_by_path( 'wind-energy-resources' ) );
          }
        
        ?>
        <div class="windswitcher__items">
            <a href="<?php echo $link . '?datatype=1&period=mean_year'; ?>" class="windswitcher__item">            
